...
@section('content')
<form action="{{ route('settings.update') }}" method="POST" style="max-width:760px">
    @csrf
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
            <div class="form-group">
                ...
                <input type="text" name="company_name" class="form-control"
                       value="{{ old('company_name', $settings['company_name'] ?? 'IIMS') }}" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="text" name="company_phone" class="form-control"
                       value="{{ old('company_phone', $settings['company_phone'] ?? '') }}" placeholder="+232-...">
            </div>
            <div class="form-group" style="grid-column:1/-1">
                <label class="form-label">Address</label>
                <input type="text" name="company_address" class="form-control"
                       value="{{ old('company_address', $settings['company_address'] ?? '') }}" placeholder="City, Country">
            </div>
        </div>
    </div>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
            <div class="form-group">
                ...
                <input type="text" name="currency_symbol" class="form-control"
                       value="{{ old('currency_symbol', $settings['currency_symbol'] ?? 'NLE') }}"
            ...
            <div class="form-group">
                ...
                <select name="fiscal_year_start_month" class="form-control" style="max-width:180px">
                    @foreach(range(1,12) as $m)
                        <option value="{{ $m }}" {{ ($settings['fiscal_year_start_month'] ?? '1') == $m ? 'selected' : '' }}>
                ...
                <div style="display:flex;align-items:center;gap:0.75rem;margin-top:0.5rem">
                    <label style="display:flex;align-items:center;gap:0.5rem;cursor:pointer;font-weight:400">
                        <input type="checkbox" name="include_saturdays" value="1"
                               {{ ($settings['include_saturdays'] ?? '0') === '1' ? 'checked' : '' }}>
    ...
    <div class="card mb-3">
        ...
        <div class="form-group">
            ...
            <textarea name="payslip_footer" class="form-control" rows="2"
                      placeholder="This is a computer-generated payslip.">{{ old('payslip_footer', $settings['payslip_footer'] ?? '') }}</textarea>
